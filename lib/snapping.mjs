...
export function snapAndResolve(moving, others, threshold = SNAP_THRESHOLD) {
  ...
  const [x, y] = pushOut({ ...moving, x: snapped.x, y: snapped.y }, others)
...
export function validate(states) {
  ...
  if (enabled.length > 1) {
    ...
    for (let i = 0; i < enabled.length; i++) {
      ...
      const merged = groups.filter(g => [...g].some(v => linked.has(v)))
    ...
    if (groups.length > 1) {
      ...
      const stranded = sorted.map(g => [...g].map(i => enabled[i].name).sort().join(", "))
