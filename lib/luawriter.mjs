 ...
 */
export function renderBlock(states, toggleBuiltin = false, keepRules = null) {
  ...
  // Rules for outputs that are not connected right now are carried across
  // rather than dropped: they are how those displays come back configured.
  if (keepRules) {
    const present = new Set(states.map(s => s.name))
    const absent = Object.keys(keepRules).filter(name => !present.has(name)).sort()
    if (absent.length) {
      lines.push("-- Not connected right now. Kept so these displays come back")
      lines.push("-- configured; delete a line to forget one.")
      for (const name of absent) {
        for (const comment of keepRules[name].comments) lines.push(comment)
        lines.push(keepRules[name].call)
      }
    }
  }
...
/**
 * The `hl.monitor` rules already inside the managed block, by output name.
 *
 * A display that is unplugged is not in `hyprctl monitors` and so is not in the
 * layout being written -- and replacing the block wholesale would drop the
 * rule that says how to set it up when it comes back. Unplugging a rotated,
 * scaled display and saving anything at all should not cost you its
 * configuration.
 */
export function rulesInBlock(text) {
  const markers = existingMarkers(text)
  if (!markers) return {}
  ...
  const found = {}
  for (const [start, stop] of callSpans(lines)) {
    const call = lines.slice(start, stop + 1).join("\n")
    const match = OUTPUT_RE.exec(call)
    if (!match || !match[1]) continue
    found[match[1]] = { call: call, comments: commentsAbove(lines, start) }
  }
  return found
}

/** The comment lines immediately above a call, which describe the display. */
function commentsAbove(lines, index) {
  const out = []
  for (let i = index - 1; i >= 0; i--) {
    const line = lines[i]
    if (/^\s*--/.test(line)) out.unshift(line)
    else break
  }
  return out
}
...
export function renderFile(existing, states, toggleBuiltin = false) {
  const keep = rulesInBlock(existing)
  return merge(existing, renderBlock(states, toggleBuiltin, keep),
               new Set(states.map(s => s.name)))
}
